...
namespace SMS.DAL
{
    public class StaffRepository
    {
        ...
        {
            _connectionString = ConfigurationManager.ConnectionStrings["SMSConnectionString"]?.ConnectionString
        ...
        public string GenerateNextTeacherCode(string branch)
        {
            ...
            using (var cmd = new SqlCommand("dbo.usp_GetNextTeacherCode", conn))
            {
                ...
                return result != null ? result.ToString() : "T0001";
            }
        }
        ...
        public (bool Success, string Message) SaveTeacher(TeacherModel model, string user)
        {
            try
            {
                ...
                using (var cmd = new SqlCommand("dbo.usp_SaveTeacher", conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@Dept_RID", model.Dept_RID ?? "DP");
                    ...
                    cmd.Parameters.AddWithValue("@User", user ?? "ADMIN");
        ...
        public List<TeacherModel> GetTeachers(string branch)
        {
            var list = new List<TeacherModel>();
            using (var conn = new SqlConnection(_connectionString))
            using (var cmd = new SqlCommand("dbo.usp_GetTeachersList", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Branch", branch);
                conn.Open();
        ...
        // ======================== GUARDIAN REPO METHODS ========================

        public string GenerateNextGuardianCode(string branch)
        {
            using (var conn = new SqlConnection(_connectionString))
            using (var cmd = new SqlCommand("dbo.usp_GetNextGuardianCode", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Branch", branch);
                conn.Open();
                var result = cmd.ExecuteScalar();
                return result != null ? result.ToString() : "G0001";
            }
        }

        public (bool Success, string Message) SaveGuardian(GuardianModel model, string user)
        {
            try
            {
                using (var conn = new SqlConnection(_connectionString))
                using (var cmd = new SqlCommand("dbo.usp_SaveGuardian", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@GuardianID", model.GuardianID);
                    cmd.Parameters.AddWithValue("@Branch", model.Branch ?? "CAP");
                    cmd.Parameters.AddWithValue("@Guardian_Code", model.Guardian_Code);
                    cmd.Parameters.AddWithValue("@FirstName", model.FirstName);
                    cmd.Parameters.AddWithValue("@MiddleName", (object)model.MiddleName ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@LastName", model.LastName);
                    cmd.Parameters.AddWithValue("@Gender", (object)model.Gender ?? DBNull.Value);
                    ...
                    cmd.Parameters.AddWithValue("@AlternatePhone", (object)model.AlternatePhone ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@WhatsAppNumber", (object)model.WhatsAppNumber ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@Email", (object)model.Email ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@AddressLine1", (object)model.AddressLine1 ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@AddressLine2", (object)model.AddressLine2 ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@City", (object)model.City ?? DBNull.Value);
                    ...
                    cmd.Parameters.AddWithValue("@Organization", (object)model.Organization ?? DBNull.Value);
                    ...
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    return (true, "Guardian profile saved successfully.");
                }
            }
            catch (Exception ex)
            {
        ...
        public List<GuardianModel> GetGuardians(string branch)
        {
            var list = new List<GuardianModel>();
            using (var conn = new SqlConnection(_connectionString))
            using (var cmd = new SqlCommand("dbo.usp_GetGuardiansList", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Branch", branch);
                conn.Open();
                using (var dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        list.Add(new GuardianModel
                        {
                            GuardianID = Convert.ToInt32(dr["GuardianID"]),
                            Branch = dr["Branch"].ToString(),
                            Guardian_Code = dr["Guardian_Code"].ToString(),
                            FirstName = dr["FirstName"].ToString(),
                            MiddleName = dr["MiddleName"] != DBNull.Value ? dr["MiddleName"].ToString() : null,
                            LastName = dr["LastName"].ToString(),
                            Gender = dr["Gender"] != DBNull.Value ? dr["Gender"].ToString() : null,
                            ...
                            AlternatePhone = dr["AlternatePhone"] != DBNull.Value ? dr["AlternatePhone"].ToString() : null,
                            WhatsAppNumber = dr["WhatsAppNumber"] != DBNull.Value ? dr["WhatsAppNumber"].ToString() : null,
                            Email = dr["Email"] != DBNull.Value ? dr["Email"].ToString() : null,
                            Occupation = dr["Occupation"] != DBNull.Value ? dr["Occupation"].ToString() : null,
                            Organization = dr["Organization"] != DBNull.Value ? dr["Organization"].ToString() : null,
                            City = dr["City"] != DBNull.Value ? dr["City"].ToString() : null,
                            State = dr["State"] != DBNull.Value ? dr["State"].ToString() : null,
                            IsActive = Convert.ToBoolean(dr["IsActive"])
                        });
        ...
        public GuardianModel GetGuardianByCode(string branch, string guardianCode)
        {
            using (var conn = new SqlConnection(_connectionString))
            using (var cmd = new SqlCommand("dbo.usp_GetGuardianByCode", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Branch", branch);
                cmd.Parameters.AddWithValue("@Guardian_Code", guardianCode);
                conn.Open();
                using (var dr = cmd.ExecuteReader())
                {
                    if (dr.Read())
                    {
                        return new GuardianModel
                        {
                            ...
                            Occupation = dr["Occupation"] != DBNull.Value ? dr["Occupation"].ToString() : null,
                            Organization = dr["Organization"] != DBNull.Value ? dr["Organization"].ToString() : null,
                            AnnualIncome = dr["AnnualIncome"] != DBNull.Value ? Convert.ToDecimal(dr["AnnualIncome"]) : (decimal?)null,
                            IsActive = Convert.ToBoolean(dr["IsActive"]),
                            PhotoUrl = dr["PhotoUrl"] != DBNull.Value ? dr["PhotoUrl"].ToString() : null,
                            Machine_Id = dr["Machine_Id"] != DBNull.Value ? dr["Machine_Id"].ToString() : null
                        };
