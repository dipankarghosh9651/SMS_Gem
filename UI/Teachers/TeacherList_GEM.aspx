    ...
    <script type="text/javascript">
        window.addEventListener('DOMContentLoaded', function () {
            ...
            (function ($) {
                ...
                var API_BASE = '<%= ResolveUrl("~/api/StudentApi/") %>';

                // Web API Lookup Loader returning a Promise
                function loadLookupFromApi(lookupType, $dropdown) {
                    return $.ajax({
                        ...
                        var items = (data && data.Data !== undefined) ? data.Data : data;
                        ...
                        if (Array.isArray(items) && items.length > 0) {
                            for (var i = 0; i < items.length; i++) {
                                ...
                                var desc = item.Desc || item.Description || code;
                ...
                function callAsmx(method, payload, cb) {
                    $.ajax({
                        ...
                        error: function (xhr) {
                            console.error("ASMX Error on " + method + ":", xhr.responseText);
                        }
                    });
                }

                // 1. Load Dropdowns FIRST using Promises to ensure availability
                var deptPromise = loadLookupFromApi("Department", $("#ddlDeptCode"));
                var genderPromise = loadLookupFromApi("Gender", $("#ddlGender"));

                // 2. Initialize Form After Lookups are Ready
                $.when(deptPromise, genderPromise).always(function () {
                    ...
                    if (editCode) {
                        ...
                        callAsmx("GetTeacherByCode", { teacherCode: editCode }, function (data) {
                            if (!data) return;
                            ...
                            $("#txtAadhaarNumber").val(data.GovtIdNumber || "");
                        ...
                        callAsmx("GetNextTeacherCode", {}, function (code) {
                            $("#txtTeacherCode").val(code);
                            $("#hdnTeacherCode").val(code);
                        });
                    }
                });
                ...
                $("#btnSaveTeacher").on("click", function () {
                    var model = {
                        ...
                        GovtIdNumber: $("#txtAadhaarNumber").val().trim(),
                    ...
                    callAsmx("SaveTeacher", { model: model }, function (res) {
                        if (res && res.Success) {
                            alert(res.Message);
